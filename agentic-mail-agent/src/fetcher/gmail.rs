...
fn extract_subject_from_headers(
    headers: &[google_gmail1::api::MessagePartHeader],
) -> Option<String> {
    headers
        .iter()
        .find(|h| {
...
fn extract_from_header(headers: &[google_gmail1::api::MessagePartHeader]) -> Option<String> {
    headers
        ...
        .find(|h| {
            h.name
                .as_ref()
                .map(|n| n.eq_ignore_ascii_case("from"))
                .unwrap_or(false)
        })
        .and_then(|h| h.value.clone())
}
...
fn extract_date_header(headers: &[google_gmail1::api::MessagePartHeader]) -> Option<String> {
    headers
        ...
        .find(|h| {
            h.name
                .as_ref()
                .map(|n| n.eq_ignore_ascii_case("date"))
                .unwrap_or(false)
        })
        .and_then(|h| h.value.clone())
}
...
impl EmailFetcher for GmailFetcher {
    async fn fetch_unread_emails(&self) -> Result<Vec<Email>, FetchError> {
        ...
        .await
        .map_err(|_| {
            FetchError::network("Gmail API call timed out after 30 seconds".to_string())
        })?;
        ...
        for (i, msg) in message_list.iter().enumerate() {
            if let Some(msg_id) = &msg.id {
                ...
                .await
                .map_err(|_| {
                    FetchError::network(format!("Gmail API call timed out for message {msg_id}"))
                })?;
...
mod tests {
    ...
    fn test_strip_html_basic() {
        ...
        assert_eq!(
            strip_html_basic("<br>Line 1<br/>Line 2<br />Line 3"),
            "Line 1\nLine 2\nLine 3"
        );
    ...
    fn test_strip_html_complex() {
        let html =
            r#"<html><body><p>Hello <strong>World</strong>!</p><br><div>Test</div></body></html>"#;
