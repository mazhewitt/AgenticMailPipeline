...
use super::{AuditLogger, FakeDataGenerator};
...
impl PiiReplacer {
        ...
        for entity in sorted_entities {
            ...
            if adjusted_start < result.len() && adjusted_end <= result.len() {
                ...
                if safe_start < safe_end && safe_end <= result.len() {
                    ...
                    if current_text.contains(&entity.text)
                        || entity.text.contains(current_text)
    ...
    fn generate_fake_value(&mut self, pii_type: &str, original_value: &str) -> String {
        ...
        let fake_value = self.fake_data_generator.generate_for_type(
            pii_type,
            original_value,
        ...
        self.replacement_cache
            .insert(original_value.to_string(), fake_value.clone());
