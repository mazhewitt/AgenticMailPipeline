...
impl FakeDataGenerator {
    ...
    fn generate_fake_email(&self, original: &str, seed: usize) -> String {
        ...
        let fake_email = format!(
            "{}{}@{}",
            usernames[username_idx], number, fake_domains[domain_idx]
        );
    ...
    fn generate_fake_location(&self, original: &str, seed: usize) -> String {
        ...
        if original.chars().next().unwrap_or(' ').is_ascii_digit() {
            ...
            format!(
                "{} {}",
                fake_postal_codes[postal_idx], fake_cities[city_idx]
            )
        } else if original.contains("mann")
            || original.contains("gasse")
            || original.contains("strasse")
            || original.contains("str")
        {
