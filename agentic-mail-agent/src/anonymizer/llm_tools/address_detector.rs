...
impl AddressDetector {
    ...
    fn find_address_in_text(&self, text: &str, llm_address: &str) -> Option<Vec<PiiEntity>> {
        ...
        for part in &address_parts {
            ...
            if part.to_lowercase() == "switzerland" || part.to_lowercase() == "germany" 
                || part.to_lowercase() == "austria" || part.to_lowercase() == "france" {
                continue;
            }
    ...
    fn find_complete_address_pattern(&self, text: &str, address_parts: &[&str]) -> Option<PiiEntity> {
        // Look for patterns like "Street Number\nPostal Code City"
        for part in address_parts {
            if part.contains("mann") || part.contains("str") || part.contains("weg") { // Street indicators
                if let Some(start) = text.find(part) {
                    ...
                    if lines.len() >= 2 {
                        ...
                        for line in &lines[1..] {
                            ...
                            if trimmed_line.len() > 4 && trimmed_line.chars().take(4).all(|c| c.is_ascii_digit()) {
