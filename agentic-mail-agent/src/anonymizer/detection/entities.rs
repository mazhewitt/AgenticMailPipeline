...
impl EntityManager {
    ...
    pub fn find_all_positions(
        &self,
        text: &str,
        llm_entities: Vec<LlmPiiEntity>,
    ) -> Vec<PiiEntity> {
        // Safety limit: prevent processing extremely large texts
        if text.len() > 1_000_000 {
            eprintln!(
                "Warning: Text too large ({} chars), truncating to 1MB",
                text.len()
            );
    ...
    fn find_all_positions_safe(
        ...
        for llm_entity in llm_entities {
            ...
            if iteration_count >= max_iterations_per_entity {
                eprintln!(
                    "Warning: Hit iteration limit for PII text: {}",
                    llm_entity.text
                );
            }
        }
            ...
            if should_add {
                ...
                deduplicated.retain(|existing| {
                    !self.entities_overlap(&entity, existing)
                        || self.should_prefer_existing(existing, &entity)
                });
