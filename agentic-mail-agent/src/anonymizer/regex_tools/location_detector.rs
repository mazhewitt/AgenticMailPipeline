...
impl LocationDetector {
    ...
    pub fn new() -> Self {
        let patterns = vec![
            ...
            Regex::new(
                r"\b[A-ZÄÖÜa-zäöüß]+(?:mann|berg|hof|weg|gasse|strasse|str\.)\s+\d+[a-z]?\b",
            )
            .unwrap(),
    ...
    fn is_valid_location(&self, location: &str) -> bool {
        ...
        if location.contains("mann")
            || location.contains("berg")
            || location.contains("hof")
            || location.contains("weg")
            || location.contains("gasse")
            || location.contains("strasse")
...
mod tests {
    ...
    fn test_swiss_postal_city() {
        ...
        assert!(locations
            .iter()
            .any(|l| l.text.contains("8700") || l.text.contains("Küsnacht")));
    }
