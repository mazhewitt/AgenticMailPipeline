...
static TRACKING_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"utm_[^&\s]*|[?&][^=]*=[^&\s]*").unwrap());

/// Remove HTML entities
static HTML_ENTITY_REGEX: Lazy<Regex> = Lazy::new(|| Regex::new(r"&[a-zA-Z0-9#]+;").unwrap());
...
fn extract_domain_from_email(email: &str) -> Option<String> {
    if let Some(at_pos) = email.rfind('@') {
        ...
        let clean_domain = domain.trim_matches(['<', '>', '"', ' ']).to_lowercase();
...
mod tests {
    ...
    fn test_clean_html_for_classification() {
        let html =
            r#"<html><body><p>Hello <a href="http://example.com">world</a>!</p></body></html>"#;
    ...
    fn test_extract_domain_from_email() {
        ...
        assert_eq!(
            extract_domain_from_email("John <john@company.org>"),
