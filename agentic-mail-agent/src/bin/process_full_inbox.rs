...
async fn main() {
    ...
    println!(
        ...
        if dry_run {
            ...
            "LIVE (will modify emails)"
        }
    );
    println!("  • Batch size: {batch_size} emails");
    ...
    let max_emails = if args.contains(&"--preview".to_string()) {
        20
    } else {
        1000
    };
    ...
    println!("📊 Found {total_emails} emails in inbox");
    ...
    println!("📈 Progress will be reported every {batch_size} emails");
    ...
    for (i, email) in emails.iter().enumerate() {
        ...
        if email_num % batch_size == 0 || email_num == total_emails {
            println!(
                ...
                email_num,
                total_emails,
                (email_num as f64 / total_emails as f64) * 100.0
            );
        }
...
fn simulate_actions(
    ...
    let archived = !matches!(classification.category, EmailCategory::ActionRequired);
