...
use agentic_mail_agent::anonymizer::{
    AnonymizationConfig, LlmBackend, PiiOrchestrator, PiiReplacer,
};
use std::fs;
...
async fn async_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "🤖 Backend: {} ({})",
        backend_name,
        args.model.as_deref().unwrap_or("default model")
    );
    ...
    if emails.is_empty() {
        println!(
            "❌ No emails found in {}. Run download_test_data first.",
        ...
        match orchestrator.detect_all_pii(&email_json).await {
            Ok(detected_entities) => {
                ...
                if pii_count != replacement_count {
                    println!(
                        "   ⚠️  Note: Detected {pii_count} PII items, replaced {replacement_count}"
                    );
                }
...
fn load_emails(input_dir: &Path) -> Result<Vec<TestEmail>, Box<dyn std::error::Error>> {
    ...
    if !manifest_path.exists() {
        return Err(format!(
            "Manifest not found: {}. Run download_test_data first.",
