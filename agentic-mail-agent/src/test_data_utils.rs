...
pub async fn anonymize_test_data(
    input_dir: &str,
    output_dir: &str,
...
pub async fn spot_check_for_pii(data_dir: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    if warnings.is_empty() {
        println!(
            "✅ PII spot check passed - no obvious PII found in {checked_count} checked files"
        );
    } else {
        println!(
            "⚠️  PII spot check found {} potential issues:",
            warnings.len()
        );
        ...
        for (field, text) in text_fields {
            ...
            if check_for_real_phones(&text) {
                warnings.push(format!(
                    "{filename}: Potential real phone number found in {field}"
                ));
            }
            ...
            if check_for_suspicious_names(&text) {
                warnings.push(format!(
                    "{filename}: Potentially real names found in {field}"
                ));
            }
        }
    }
