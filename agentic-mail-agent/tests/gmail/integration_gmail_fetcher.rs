...
async fn test_gmail_fetcher_subject_and_body() {
    ...
    let emails = fetcher
        .fetch_unread_emails()
        .await
        .expect("Failed to fetch unread emails");
    ...
    for email in &emails {
        ...
        assert!(
            email.subject.is_some(),
            "Email {} missing subject",
            email.id
        );
        assert!(
            email.snippet.is_some(),
            "Email {} missing snippet",
            email.id
        );
    }
...
async fn test_gmail_auth_issue_isolation() {
    ...
    if with_neither > 0 {
        println!(
            "⚠️  Found {with_neither} emails with missing subject AND snippet - likely auth issue"
        );
    }
    ...
    for email in &emails {
        ...
        match (&email.subject, &email.snippet) {
            (Some(subject), Some(_snippet)) => {
                println!(
                    "✅ Email {} has subject '{}' and snippet",
                    email.id, subject
                );
            }
            (Some(subject), None) => {
                println!(
                    "⚠️  Email {} has subject '{}' but missing snippet",
                    email.id, subject
                );
            }
            (None, Some(_)) => {
            ...
            (None, None) => {
                println!(
                    "❌ Email {} missing both subject and snippet (likely auth failure)",
                    email.id
                );
            }
        }
            ...
            if std::path::Path::new(&path).exists() {
                ...
                if let Ok(token_content) = std::fs::read_to_string(&path) {
                    if let Ok(token_json) =
                        serde_json::from_str::<serde_json::Value>(&token_content)
                    {
                        println!(
                            "  📄 Token file contains {} keys",
                        ...
                        if let Some(obj) = token_json.as_object() {
                            ...
                            let has_expires =
                                obj.contains_key("expires_in") || obj.contains_key("expires_at");

                            println!(
                                "    - Has access_token: {}",
                                if has_access_token { "✅" } else { "❌" }
                            );
                            println!(
                                "    - Has refresh_token: {}",
                                if has_refresh_token { "✅" } else { "❌" }
                            );
                            println!(
                                "    - Has expiry info: {}",
                                if has_expires { "✅" } else { "❌" }
...
async fn test_gmail_minimal_auth_operation() {
    ...
    let https_connector =
        match google_gmail1::hyper_rustls::HttpsConnectorBuilder::new().with_native_roots() {
            Ok(builder) => builder,
            Err(e) => {
                        ...
                        if let Some(messages) = response.messages {
                            if let Some(first_msg) = messages.first() {
                                if let Some(msg_id) = &first_msg.id {
                                    println!(
                                        "   📧 Testing individual message fetch for ID: {msg_id}"
                                    );
                                    match hub
                                        .users()
                                        .messages_get("me", msg_id)
