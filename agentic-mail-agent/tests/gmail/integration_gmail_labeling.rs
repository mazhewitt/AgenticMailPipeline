...
use agentic_mail_agent::action::impls::labeler::{EmailLabeler, GmailLabeler, LabelingError};
...
async fn test_gmail_labeler_create_and_apply_label() {
    ...
    let labeler = GmailLabeler::from_env()
        .await
    ...
    // Apply the label
    let result = labeler
        .apply_label(test_message_id, test_label)
        .await
...
async fn test_gmail_labeler_ensure_label_exists() {
    ...
    // Ensure label exists
    let label_id = labeler
        .ensure_label_exists(test_label)
        .await
