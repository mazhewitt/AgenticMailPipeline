...
async fn test_pii_detection_functions() {
    ...
    fs::write(
        format!("{test_dir}/email_with_pii.json"),
        test_email_with_pii,
    )
    .unwrap();
    fs::write(
        format!("{test_dir}/email_anonymized.json"),
...
fn test_pii_detection_patterns() {
    ...
    assert!(agentic_mail_agent::test_data_utils::check_for_real_emails(
        "Contact me at real.person@company.com"
    ));
    assert!(!agentic_mail_agent::test_data_utils::check_for_real_emails(
    ...
    assert!(agentic_mail_agent::test_data_utils::check_for_real_phones(
    ...
    assert!(
        agentic_mail_agent::test_data_utils::check_for_suspicious_names(
