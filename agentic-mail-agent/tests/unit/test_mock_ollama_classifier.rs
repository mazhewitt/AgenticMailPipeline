...
async fn test_mock_classification_contains_reasoning() {
    ...
    assert!(classification.llm_response.to_lowercase().contains("scam") || 
           classification.llm_response.to_lowercase().contains("suspicious")); // Should identify spam characteristics
}