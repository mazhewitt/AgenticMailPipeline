...
fn test_complete_email_anonymization_workflow() {
  ...
  "file_index": 1
    ...
    let anonymized_text = replacer
        .replace_pii(&full_text, &detected_entities)
        .unwrap();
    ...
    for entry in replacement_log {
        println!(
            "  {} '{}' → '{}'",
            entry.pii_type, entry.original_value, entry.fake_value
        );
    }
    println!();
    ...
    println!(
        "{}",
        serde_json::to_string_pretty(&anonymized_email).unwrap()
    );
...
fn test_fallback_detection_on_missed_pii() {
    ...
    let partial_llm_entities = vec![PiiEntity {
    ...
    let mut replacer = PiiReplacer::new();
    let result = replacer
        .replace_pii(text_with_mixed_pii, &partial_llm_entities)
        .unwrap();
