...
async fn test_complete_workflow_with_full_email_fields() {
    ...
    let routing_result = router
        .route(email, &classification)
        .await
        .expect("Should route email");

    // Verify urgent email detection works with the new body field
    assert!(
        routing_result.has_high_priority_actions(),
        "Should detect as urgent due to body content"
    );
...
async fn test_classification_uses_all_email_fields() {
    ...
    assert_eq!(
        work_email.to_or_default(),
        vec!["dev-team@company.com".to_string()]
    );
    assert_eq!(
        work_email.sent_or_default(),
        "Wed, 30 Jun 2023 09:00:00 +0000"
    );
    assert!(work_email.body_or_default().contains("deadline"));

    assert_eq!(personal_email.from_or_default(), "friend@personal.com");
    assert_eq!(
        personal_email.to_or_default(),
        vec!["me@personal.com".to_string()]
    );
...
async fn test_urgent_detection_in_body_content() {
    ...
    // Route the email
    let routing_result = router
        .route(&urgent_body_email, &classification)
        .await
        .unwrap();
    ...
    assert!(
        routing_result.has_high_priority_actions(),
        "Should detect urgency in body content"
    );
    assert!(routing_result.reasoning.contains("urgent content detected"));
}
...
fn test_email_field_accessors() {
    ...
    let full_email = Email::new_full(
        ...
        Some(vec![
            "recipient1@test.com".to_string(),
            "recipient2@test.com".to_string(),
        ]),
    ...
    assert_eq!(
        full_email.sent_or_default(),
        "Wed, 30 Jun 2023 12:00:00 +0000"
    );
