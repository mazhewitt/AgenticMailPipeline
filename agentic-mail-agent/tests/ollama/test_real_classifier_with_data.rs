...
mod tests {
    ...
    async fn test_real_classifier_with_sample_emails() {
        ...
        let sample_emails: Vec<Email> = test_emails.into_iter()
        ...
        for (i, email) in sample_emails.iter().enumerate() {
            ...
            match classifier.classify(email).await {
                Ok(classification) => {
                    ...
                    assert!(!classification.llm_response.is_empty(), "LLM response should not be empty");
        ...
        assert!(success_rate >= 0.5, "Should achieve at least 50% classification success rate");
    ...
    async fn test_classifier_category_distribution() {
        ...
        for email in &emails {
            match classifier.classify(email).await {
                ...
                Err(e) => {
                    println!("   ❌ Failed to classify '{}': {}", email.subject_or_default(), e);
                }
            }
        }
    ...
    async fn test_classifier_consistency() {
        ...
        if let Some(test_email) = test_emails.first() {
            ...
            if classifications.len() >= 2 {
                ...
                let consistent = classifications.iter().all(|c| &c.category == first_category);
