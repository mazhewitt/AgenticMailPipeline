...
impl TestDataEmail {
    ...
    fn to_email(&self) -> Email {
        Email::new(self.id.clone(), self.subject.clone(), self.snippet.clone())
    }
}
...
mod tests {
    ...
    async fn test_prepare_data_for_classifier() {
        ...
        match load_all_test_emails() {
            Ok(test_emails) => {
                println!(
                    "🧪 Preparing {} real emails for classification",
                    test_emails.len()
                );
                ...
                println!(
                    "💡 You can now use these Email objects with MessageClassifier::classify()"
                );
            }
            Err(e) => {
                println!("⚠️  Could not load test data: {e}");
    ...
    fn test_data_directory_structure() {
        ...
        if test_data_dir.exists() {
            ...
            assert!(
                ...
                "Should have some email_*.json files"
            );
