...
use agentic_mail_agent::{
    action::impls::labeler::{ConcreteGmailLabeler, EmailLabeler, LabelingError, LabelingResult},
...
impl Drop for TestCleanupGuard {
    fn drop(&mut self) {
        if self.active {
            println!(
                "⚠️  TestCleanupGuard: Cleanup required on drop - some test labels may remain"
            );
...
fn extract_category_from_test_label(test_label: &str) -> Option<&str> {
    let label_config = LabelConfig::new();
    if label_config.is_test_label(test_label) {
        test_label.strip_prefix(&label_config.test.prefix)
    } else {
        None
    }
}
...
async fn cleanup_test_labels_concurrently(
    ...
    // Find test labels using centralized configuration
    let label_config = LabelConfig::new();
    let test_labels: Vec<_> = all_labels
        .iter()
        .filter(|label| label_config.is_test_label(&label.name))
        .collect();
    ...
    for chunk in test_labels.chunks(MAX_CONCURRENT_REQUESTS) {
        ...
        for label in chunk {
            ...
            futures.push(async move {
                let result = retry_with_backoff(
                    || async {
                        labeler_clone.delete_label(&label_id).await.map_err(|e| {
                            LabelingError::unknown(format!("Failed to delete label: {e}"))
                        })
                    },
                    rate_limiter_ref,
...
async fn cleanup_test_labels(
    labeler: &ConcreteGmailLabeler,
...
async fn cleanup_test_labels_with_retries(
    labeler: &ConcreteGmailLabeler,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  📊 Cleanup completed: {} succeeded, {} failed",
        ...
        match operation_with_timeout.await {
            ...
            Err(error_msg) => {
                if attempt < RETRY_ATTEMPTS {
                    ...
                    println!(
                        "  ❌ {operation_name} failed after {RETRY_ATTEMPTS} attempts: {error_msg}"
                    );
...
async fn test_gmail_auth_setup() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !test_labels.is_empty() {
        println!(
            "  ⚠️  Found {} existing test labels - consider running cleanup",
            test_labels.len()
        );
...
async fn test_classifier_labeller_integration_full_workflow() {
    ...
    let test_future = async {
        ...
        for (i, email) in test_emails.iter().enumerate() {
            ...
            let classification = classifier.classify(email).await.unwrap_or_else(|_| {
                panic!(
                    "Failed to classify {}",
                    format_email_context(&email.id, &email.subject)
                )
        ...
        let labeling_results =
            apply_labels_concurrently(Arc::clone(&labeler), emails_with_classifications.clone())
                .await;
        ...
        assert_eq!(
            labeling_results.failure_count(),
            0,
            "All label applications should succeed. Failures: {:#?}",
            labeling_results.failures
        );
        assert_eq!(
            labeling_results.success_count(),
            emails_with_classifications.len(),
            "Should have applied labels to all {} emails",
            emails_with_classifications.len()
        );

        // Step 6: Verify labels concurrently
        println!("\n🔍 Step 6: Verifying labels concurrently...");
        let email_label_pairs: Vec<_> = emails_with_classifications
            .iter()
            .map(|(email, _, test_label)| (email.clone(), test_label.clone()))
            .collect();

        let verification_results =
            verify_labels_concurrently(Arc::clone(&labeler), email_label_pairs).await;
            ...
            match &attempt.result {
                Ok(false) => {
                    failure_messages.push(format!(
                        ...
                        context, attempt.expected_label
                    ));
                }
                Err(e) => {
        ...
        assert_eq!(
            verification_results.failure_count(),
            0,
        ...
        let idempotency_test_items: Vec<_> = emails_with_classifications
            .iter()
            .take(3)
            .cloned()
            .collect();
        let idempotency_results =
            apply_labels_concurrently(Arc::clone(&labeler), idempotency_test_items.clone()).await;
            ...
            if let Ok(result) = &attempt.result {
                if result.created_new_label {
                    idempotency_failures.push(format!(
                        "Re-application of label '{}' to {} created new label",
                        attempt.label,
                        format_email_context(&attempt.email_id, &attempt.email_subject)
                    ));
                }
            }
        }
        ...
        println!(
            ...
            verification_results.success_count()
        );
        println!(
            "  - Classification categories used: {}",
            classification_summary.len()
        );

        // Final assertions
        assert!(
            !emails_with_classifications.is_empty(),
            "Should have processed at least one email"
        );
        assert!(
            !classification_summary.is_empty(),
...
async fn test_classifier_with_real_emails_quality_assessment() {
    ...
    println!(
        "📊 Analyzing {} emails for classification quality",
        test_emails.len()
    );
    ...
    // Quality checks
    let high_score_count = valid_scores.iter().filter(|&&score| score >= 0.8).count();
    let low_score_count = valid_scores.iter().filter(|&&score| score < 0.5).count();

    println!("\nQuality metrics:");
    println!(
        "  - High score (≥0.8): {} emails ({}%)",
        high_score_count,
        if !valid_scores.is_empty() {
            (high_score_count * 100) / valid_scores.len()
        } else {
            0
        }
    );
    println!(
        "  - Low score (<0.5): {} emails ({}%)",
        low_score_count,
        if !valid_scores.is_empty() {
            (low_score_count * 100) / valid_scores.len()
        } else {
            0
        }
    );
    ...
    assert!(
        category_counts.len() > 1,
        "Should classify emails into multiple categories, got {}",
        category_counts.len()
    );
...
async fn test_labeller_label_management() {
    ...
    let label_config = LabelConfig::new();
    let our_test_labels: Vec<_> = all_labels
        .iter()
        .filter(|label| label_config.is_test_label(&label.name))
        .collect();
    ...
    for label in &our_test_labels {
        ...
        assert!(
            test_labels.contains(&label.name),
            "Found unexpected test label: {}",
            label.name
        );
    }
    ...
    let label_config = LabelConfig::new();
    let remaining_test_labels: Vec<_> = all_labels_after
        .iter()
        .filter(|label| label_config.is_test_label(&label.name))
        .collect();

    assert_eq!(
        remaining_test_labels.len(),
        0,
...
async fn test_end_to_end_workflow_with_cleanup() {
    ...
    println!(
        "🔄 Processing {} emails through complete workflow",
        test_emails.len()
    );
    ...
    let email_label_pairs: Vec<_> = processed_emails
        .iter()
        .map(|(email, _, test_label)| ((*email).clone(), test_label.clone()))
        .collect();
    ...
    assert_eq!(
        ...
        "Should verify labels on all {} emails",
        processed_emails.len()
    );
    ...
    let test_labels_used: HashSet<_> = processed_emails
        .iter()
    ...
    println!(
        "   Successfully processed {} emails through the complete pipeline",
        processed_emails.len()
    );
}
