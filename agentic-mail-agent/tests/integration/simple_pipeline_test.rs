...
async fn test_simple_end_to_end_pipeline() {
    ...
    for (i, email) in test_emails.iter().enumerate() {
        ...
        let test_label = format!("{}{}", TEST_LABEL_PREFIX, classification.category.to_uppercase());
        
        // Apply the label
        let result = labeler.apply_label(&email.id, &test_label).await
    ...
    for (email, expected_label) in test_emails.iter().zip(applied_labels.iter()) {
        let email_labels = labeler.get_email_labels(&email.id).await
    ...
    let test_labels_to_delete: Vec<_> = all_labels.iter()
    ...
    println!("   Processed {} emails through the complete workflow", test_emails.len());
}
