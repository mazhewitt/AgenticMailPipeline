...
fn classify_email_content(subject: &str, from: &str, body: &str) -> String {
    ...
    // Security alerts
    if subject_lower.contains("security alert")
        || from_lower.contains("no-reply@accounts.google.com")
    {
        return "security".to_string();
    }

    // Newsletters
    if from_lower.contains("flipboard")
        || from_lower.contains("nytimes")
        || from_lower.contains("uefa")
    {
        ...
        match load_all_test_emails() {
            Ok(emails) => {
                ...
                println!(
                    "   • With subject: {} ({:.1}%)",
                    with_subject,
                    (with_subject as f64 / total as f64) * 100.0
                );
                println!(
                    "   • With snippet: {} ({:.1}%)",
                    with_snippet,
                    (with_snippet as f64 / total as f64) * 100.0
                );
                println!(
                    "   • With from: {} ({:.1}%)",
                    with_from,
                    (with_from as f64 / total as f64) * 100.0
                );
                println!(
                    "   • With to: {} ({:.1}%)",
                    with_to,
                    (with_to as f64 / total as f64) * 100.0
                );
                println!(
                    "   • With body: {} ({:.1}%)",
                    with_body,
                    (with_body as f64 / total as f64) * 100.0
                );
                println!(
                    "   • With sent: {} ({:.1}%)",
                    with_sent,
                    (with_sent as f64 / total as f64) * 100.0
                );

                // Assert minimum quality thresholds
                assert!(
                    with_subject >= (total * 90 / 100),
                    "At least 90% of emails should have subjects"
                );
                assert!(
                    with_from >= (total * 95 / 100),
                    "At least 95% of emails should have from addresses"
                );
                assert!(
                    with_snippet >= (total * 80 / 100),
                    "At least 80% of emails should have snippets"
                );
    ...
    fn test_data_quality_metrics() {
        match load_all_test_emails() {
            Ok(emails) => {
                ...
                // Quality assertions
                assert!(
                    avg_subject_len > 10.0,
                    "Average subject length should be meaningful"
                );
                assert!(
                    avg_snippet_len > 20.0,
                    "Average snippet length should be meaningful"
                );
