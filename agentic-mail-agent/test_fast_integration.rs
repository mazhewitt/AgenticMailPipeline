...
async fn main() {
    ...
    let fetcher = GmailFetcher::from_env().await.expect("Failed to create fetcher");
    let labeler = ConcreteGmailLabeler::from_env().await.expect("Failed to create labeler");
    ...
    let emails = fetcher.fetch_unread_emails().await.expect("Failed to fetch emails");
    ...
    // Classify the email
    println!("\n🎯 Classifying email...");
    let classification = classifier.classify(test_email).await.expect("Failed to classify");
    let test_label = create_test_label(&classification.category);
    println!("  ✅ Classification: {} -> {}", classification.category, test_label);
    
    // Apply label (no rate limiting)
    println!("\n🏷️  Applying label directly...");
    match timeout(Duration::from_secs(10), labeler.apply_label(&test_email.id, &test_label)).await {
        Ok(Ok(result)) => {
    ...
    match timeout(Duration::from_secs(10), labeler.get_email_labels(&test_email.id)).await {
        Ok(Ok(labels)) => {
    ...
    let all_labels = labeler.list_all_labels().await.expect("Failed to list labels");
    let test_labels: Vec<_> = all_labels.iter()
        .filter(|l| l.name.starts_with(TEST_LABEL_PREFIX))
        .collect();
