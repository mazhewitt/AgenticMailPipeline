...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let client_secret_path = env::var("GMAIL_CLIENT_SECRET_JSON")
        .unwrap_or_else(|_| {
            println!("⚠️  GMAIL_CLIENT_SECRET_JSON not set, using default: ./secrets/client-secret.json");
    ...
    println!("📄 Read {} bytes from client secret file", secret_content.len());
    ...
    let secret: yup_oauth2::ApplicationSecret = {
        ...
        if let Some(installed) = google_secret.get("installed") {
            ...
            serde_json::from_value(installed.clone())
                .map_err(|e| format!("Failed to parse client secret from 'installed' section: {}", e))?
        } else {
...
fn create_env_script(client_secret_path: &str, token_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    let env_script = format!(
        r#"#!/bin/bash
# Gmail API Environment Variables
# Generated by auth_setup utility

export GMAIL_CLIENT_SECRET_JSON="{}"
